...
const CHOICES = {
    ...
    1: [
        '<button class=choice id=choice-1-2 value=2>Refuse to investigate, pass it off as the fancy of your tired mind</button>',
        '<button class=choice id=choice-1-3 value=3>Rise, don coat, hat, and cane, and step out of your parlor to see what the trouble is</button>'
    ],
    2: [
        '<button class=choice id=choice-2-4 value=4>Return to your bed and seek the solace of your ratted down comforter, lighting every lamp and praying to the God whose existence you have forsworn</button>',
        '<button class=choice id=choice-2-3 value=3>Dress and venture into the hallway, facing what you hope to be imaginary fear</button>'
    ]
};
...
function choiceEventListenerAdd() {
    ...
    choiceBtn.addEventListener('click', () => {makeChoice(choiceBtn.value)});
}
...
function makeChoice(input) {
    ...
    choicesDiv.innerHTML = '';
    if (CHOICES[input].length > 1) {
        for (let i = 0; i < CHOICES[input].length; i++) {
            choicesDiv.innerHTML += CHOICES[input][i];
        }
    } else {
